//包含了mystl的所有基本算法
// Created by new on 2020/5/22.
//

#ifndef UNTITLED_ALGOBASE_H
#define UNTITLED_ALGOBASE_H
...
#ifdef min
#pragma message("#underfing marco min")
#undef min
#endif //min
...
template <class RandomIter, class OutputIterator>
OutputIterator
unchecked_copy_cat(RandomIter first, RandomIter last, OutputIterator result,
        mystl::is_random_access_iterator)
    {
...
/***********************************************************/
//unchecked_copy_backward_cat 的 bidirectional_iterator_tag版本
...
//unchecked_copy_backward_cat的 random_access_iterator_tag版本
template <class BidirectionalIter1, class BidirectionalIter2>
BidirectionalIter2
unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
...
/***********************************************************/
//copy_if
//把[first, last)内满足一元操作 unary_pred 的元素拷贝到以result 为起始的位置上
/***********************************************************/
...
/***********************************************************/
//move_backward
//将[first, last)区域内的元素移动到[result - (last - first), result)内
/***********************************************************/
//bidirectional_iterator_tag版本
template<class BidirectionalIter1, class BidirectionalIter2>
BidirectionalIter2
unchecked_move_backward_cat(BidirectionalIter1 first, BidirectionalIter1 first,
        BidirectionalIter2 result, mystl::bidirectional_iterator_tag)
{
    while(first != last)
...
BidirecitonalIter2
unchecked_move_backward(BidirecitonalIter1 first, BidirecitonalIter1 last,
        BidirecitonalIter2 result)
{
...
/***********************************************************/
//equal
//比较第一序列在[first, last )区间上的元素是否与第二序列相等
/***********************************************************/
...
unchecked_fill_n(Tp* first, Size n, Up value)
    ...
    if(n > 0)
    {
        std::meset(first, (unsigned char)value, (size_t)(n));
    }
    return first + n;
    }
...
//以字典序排列对两个序列进行比较，当在某个位置发现第一组不相等元素时，有以下集中情况：
//（1）如果第一序列的元素较小，返回true，否则返回false
//（2）如果到达last1而尚未到达last2返回true
//（3）如果到达last2而尚未到达last1返回false
...
bool lexicographical_compare(InputIter1 first1, InputIter1 last1,
    ...
    {
    for(; first1 != las1 && firs2 != last2; ++first1, ++first2)
    {
        if(*first1 < *first2)
            return true;
        if(*first2 < *first1)
...
/***********************************************************/
//mismatch
//平行比较两个序列，找到第一处失匹配的元素，返回一堆迭代器，分别指向两个序列中失配的元素
/***********************************************************/
template <class InputIter1, class InputIter2>
