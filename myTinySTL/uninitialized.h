...
#ifndef UNTITLED_UNINITIALIZED_H
#define UNTITLED_UNINITIALIZED_H
...
//把[first, last+n)上的内容复制到以result为起始位置的空间，返回复制结束后的位置
/**********************************************************************/
...
template <class FordwardIter, class T>
void 
unchecked_uninit_fill(ForwardIter first, FordwardIter last, const T& value, std::false_type)
{
    auto cur = first;
    try {
        for(; cur != last; ++cur)
...
template <class ForwardIter, class Size, class T>
ForwardIter
unchecked_uninit_fill_n(ForwardIter first, Size n, __const T& value, std::false_type)
{
    auto cur = first;
    try {
        for(; n > 0; --n, ++cur)
...
unchecked_uninit_move_n(InputIter first, Size n, ForwardIter result, std::false_type)
{
    ...
    try {
        for(; n > 0 ; --n, ++first, ++cur)
        {
            mystl::construct(&*cur, mystl::(*first));
        }
    }
    catch (...)
    {
...
#endif //UNTITLED_UNINITIALIZED_H
