//包含通用工具的头文件：move，forward，swap等
// Created by new on 2020/5/5.
//
...
namespace mystl{
    ...
    {
        return statice_cast<T&&>(arg);
    }

    template <class T>
    T&& forward(typename std::remove_reference<T>::type&& arg) noexcept
    {
        static_assert(!std::remove_reference<T>::value, "bad forward");
    ...
    //结构体模版：pair
    //两个模版参数分别表示两个数据的类型
    //用first 和second 来分别取出第一个数据和第二个数据
    ...
    struct pair
    {
            ...
            template<class Other1 = Ty1, class Other2 = Ty2, typename = typename std::enable_if<
                    ...
                    std::is_default_constructible<Othter2>::value, void>::type>
                    constexpr pair()
                    : first(), second()
            {
            ...
            template <class Other1, class Other2,
                    typename std::enable_if<
                            ...
                            (!std::is_convertible<const Other1&, Ty1>::value ||
                             !std::is_convertible<const Other2&, Ty2>::value), int>::type = 0>
            explicit constexpr pair(const pair<Other1, Other2>& other)
                    :fisrt(mystl::forward<Other1>(other.first)),
                     second(mystl::forward<Other2>(other.second))
            {
            }
